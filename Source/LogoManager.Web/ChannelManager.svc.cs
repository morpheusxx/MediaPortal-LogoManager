...
namespace ChannelManager
{
    ...
    {
        public Dictionary<string, string> GetLogos(List<string> channelsNames, string regionCode)
        {
            ...
            using (var ctx = new EF.RepositoryContext("LogoDB"))
            {
                ...
                {
                    var aliases = ctx.Aliases.Include("Channel").Include("Channel.Logos").Include("Channel.Logos.Suggestion").Where(a => a.Name == channelName);
                    if (aliases.Any())
                    {
